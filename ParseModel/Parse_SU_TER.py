...
def parse_location_upload_su_ter(data):
    alarm_flag = byte2str(data[13:17])
    ...
    logger.debug('—————— 报警标识 {} 状态 {} 速度 {} km/h {} 纬度 {} 经度 {} 时间 {} ——————'.format(alarm_flag, state, speed/10, gps_state, latitude, longitude, alarm_time))
    if len(data) > 53:
...
def parse_query_para_su_ter(data):
    ...
    para_num = big2num(byte2str(data[15:16]))
    logger.debug('参数个数 {}'.format(para_num))
    temp = 0
    for n in range(para_num):
        para_id = byte2str(data[16+temp:20+temp])
        para_len = big2num(byte2str(data[20+temp:20+temp+1]))
        para_content = data[20+temp+1:20+temp+1+para_len]
        temp += para_len + 5
